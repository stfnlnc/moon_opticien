        ...
        <div id="search-results" class="grid grid--2 grid-gap--6">
            ...
            <div>
                <div class="p--2 border--rounded bg--secondary-light mb--8">
                            ...
                            <div class="w--100 p--2">
                                <div class="flex row align--center gap--1 text--s hide-mobile w--100">
                                    <form method="post" action="{{ route('options.update', $option) }}"
                                        class="flex col gap--2 w--100">
                                        @csrf
                                        @method('patch')
                                        <x-input-label for="{{ $option->options_key }}" :value="__($option->options_name)" />
                                        <x-text-input class="w--100" id="{{ $option->options_key }}"
                                            name="options_value" type="text" :value="old('options_value', $option->options_value)" required autofocus
                                            autocomplete="options_value" />
                    ...
                    @endforeach
                    <div class="p--2 mt--8">
                        <h5>Adresse Denguin</h5>
                    </div>
                    @foreach ($options as $key => $option)
                        @if ($option->options_category === 'address_2')
                            <div class="w--100 p--2">
                                <div class="flex row align--center gap--1 text--s hide-mobile w--100">
                                    <form method="post" action="{{ route('options.update', $option) }}"
                                        class="flex col gap--2 w--100">
                ...
                <div class="p--2 border--rounded bg--secondary-light mb--8">
                    ...
                    @endforeach
                    <div class="p--2 mt--8">
                        <h5>Informations Denguin</h5>
                    </div>
                    @foreach ($options as $key => $option)
                        @if ($option->options_category === 'informations_2')
                            <div class="w--100 p--2 border--rounded {{ $key % 2 != 0 ? 'bg--secondary-light' : '' }}">
                                <div class="flex row align--center gap--1 text--s hide-mobile w--100">
                                    <form method="post" action="{{ route('options.update', $option) }}"
                                        class="flex col gap--2 w--100">
                            ...
                            <div class="flex row align--center gap--1 text--s hide-mobile w--100">
                                <form method="post" action="{{ route('options.update', $option) }}"
                                    ...
                                    <div class="flex row gap--2 align--center">
                                        @php
                                            ...
                                            if ($hours[0] !== '') {
                                                if (isset($hours) && count($hours) === 2) {
                                                    ...
                                                    $openPm = $Pm[0];
                                                    $closePm = $Pm[1];
                                                } elseif ($hours[0] !== 'Fermé' && count($hours) === 1) {
                                        ...
                                        @endphp
                                        <input class="form-input w--fit-content" value="{{ $openAm }}"
                                            type="time" name="open_am_{{ $option->options_key }}"
                                            id="open_am_{{ $option->options_key }}">
                                        <p> - </p>
                                        <input class="form-input w--fit-content" value="{{ $closeAm }}"
                                            type="time" name="close_am_{{ $option->options_key }}"
                                            id="close_am_{{ $option->options_key }}">
                                        <p> et </p>
                                        <input class="form-input w--fit-content" value="{{ $openPm }}"
                                            type="time" name="open_pm_{{ $option->options_key }}"
                                            id="open_pm_{{ $option->options_key }}">
                                        <p> - </p>
                                        <input class="form-input w--fit-content" value="{{ $closePm }}"
                                            type="time" name="close_pm_{{ $option->options_key }}"
                                            id="close_pm_{{ $option->options_key }}">
                                        <script>
                                            addEventListener('submit', () => {
                                                ...
                                                document.getElementById('{{ $option->options_key }}').value = (openAm ? (openAm + '–') : '') +
                                                    closeAm + ((closeAm && openPm) ? ', ' : '') + (openPm ? (openPm + '–') : '') + (closePm ? closePm :
                                                        '')
                                        ...
                                        <input hidden type="text" name="options_value"
                                            id="{{ $option->options_key }}">
                                    </div>
                                    <x-input-error :messages="$errors->get('options_value')" />
                                    <x-primary-button>{{ __('Modifier') }}</x-primary-button>
                                </form>
