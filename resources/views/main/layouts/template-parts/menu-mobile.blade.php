<div class="flex row col-mobile align-mobile--start align--center justify--center gap--4 gap--4-mobile">
    ...
    <a href="{{ route('home_service') }}"
       class="btn--navbar-{{ $mode }} @if (str_contains($route, 'home_service')) btn--navbar-{{ $mode }}--active @endif">À
        Domicile</a>
    <div class="btn--navbar-{{ $mode }} @if (str_contains($route, 'glasses') OR str_contains($route, 'lenses') OR str_contains($route, 'audition')) btn--navbar-{{ $mode }}--active @endif">
        Nos Produits
        <div class="w--fit-content border--rounded flex col gap--2 pl--4 pt--2">
            <a href="{{ route('glasses') }}"
               class="w-100 btn--navbar-dark @if (str_contains($route, 'glasses')) btn--navbar-dark--active @endif">Lunettes</a>
            <a href="{{ route('lenses') }}"
               class="w-100 btn--navbar-dark @if (str_contains($route, 'lenses')) btn--navbar-dark--active @endif">Lentilles</a>
            <a href="{{ route('audition') }}"
               class="w-100 btn--navbar-dark @if (str_contains($route, 'audition')) btn--navbar-dark--active @endif">Audition</a>
        </div>
    </div>
    <a href="{{ route('about') }}"
